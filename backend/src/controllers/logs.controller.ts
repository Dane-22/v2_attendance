...
export const getLogs = async (
  ...
  try {
    ...
    // Action type filter
    if (actionTypes) {
      const types = (actionTypes as string).split(',').filter(t => VALID_ACTION_TYPES.includes(t));
      if (types.length > 0) {
    ...
    // Entity type filter
    if (entityTypes) {
      const types = (entityTypes as string).split(',').filter(t => VALID_ENTITY_TYPES.includes(t));
      if (types.length > 0) {
    ...
    // Status filter
    if (status) {
      const statuses = (status as string).split(',').filter(s => VALID_STATUSES.includes(s));
